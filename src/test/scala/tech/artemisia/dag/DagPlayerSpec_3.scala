...
class DagPlayerSpec_3 extends ActorTestSpec {
  ...
  it must "import and iteration in single test" in {
    ...
    within(20000 millis) {
      ...
      probe.validateAndRelay(dag_player) {
        case TaskSuceeded("task1$1$step1", stats: TaskStats) => {
          stats.status must be(Status.SUCCEEDED)
      ...
      probe.validateAndRelay(dag_player) {
        case TaskSuceeded("task1$1$step2", stats: TaskStats) => {
          stats.status must be(Status.SUCCEEDED)
      ...
      probe.validateAndRelay(dag_player) {
        case TaskSuceeded("task1$2$step1", stats: TaskStats) => {
          stats.status must be(Status.SUCCEEDED)
      ...
      probe.validateAndRelay(dag_player) {
        case TaskSuceeded("task1$2$step2", stats: TaskStats) => {
          stats.status must be(Status.SUCCEEDED)
